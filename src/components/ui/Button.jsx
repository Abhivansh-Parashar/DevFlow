...
const VARIANTS = {
  primary: 'btn-primary text-white dark:text-[#04120E]',
...
export function Button({ variant = 'primary', size = 'md', icon: Icon, iconRight: IconRight, className, children, ...rest }) {
  return (
    <button
      className={cx(
        'focus-ring inline-flex select-none items-center justify-center rounded-lg font-medium transition-all duration-200 ease-out active:scale-[0.97] disabled:pointer-events-none disabled:opacity-50',
        VARIANTS[variant],
        SIZES[size],
        className
      )}
...
export function IconButton({ label, icon: Icon, size = 16, className, ...rest }) {
  return (
    <button
      aria-label={label}
      title={label}
      className={cx('focus-ring grid h-8 w-8 place-items-center rounded-lg text-muted transition-all duration-200 active:scale-90 hover:bg-raised hover:text-ink', className)}
      {...rest}
    >
